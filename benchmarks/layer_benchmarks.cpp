...
static void BM_layer_forward(benchmark::State& state) {
    ...
    vi::nn::layer layer(context, new vi::nn::sigmoid_activation(), weights);
...
static void BM_layer_backward(benchmark::State& state) {
    ...
    while (state.KeepRunning()) {
        std::pair<vi::la::matrix, vi::la::matrix> delta_and_gradient = layer.backward(inputs, activations, delta);
    }
