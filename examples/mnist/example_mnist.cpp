...
pair<vi::la::matrix, vi::la::matrix> load_svm_dataset(vi::la::context& context, const string& path,
...
int main(int, const char**) {
  ...
  const auto labels_and_features = load_svm_dataset(context, training_set_path, feature_count);
  ...
  auto test_labels_and_features = load_svm_dataset(context, test_set_path, feature_count);
  ...
  const vector<size_t> all_indices = generate_random_indices(all_features.row_count());
  const size_t training_example_count = training_fraction * all_indices.size();
  const vector<size_t> training_indices(all_indices.begin(),
                                        all_indices.begin() + training_example_count);
  const vector<size_t> validation_indices(all_indices.begin() + training_example_count,
                                          all_indices.end());
  ...
  minibatch.set_stop_early([&](const network& nw, size_t epoch, double cost) -> bool {
    ...
    measurements.add_results(validation_labels, map.activations_to_labels(predictions));
    cout << "epoch: " << epoch << ", cost: " << cost << ", accuracy: " << measurements.accuracy()
         << endl;
  ...
  result_measurements measurements(context, map.labels());
  measurements.add_results(testing_labels, map.activations_to_labels(predictions));
