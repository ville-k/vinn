...
libsvm_file::load_labels_and_features(vi::la::context& context,
  ...
  labels_and_features_vector rows = parse_contents(
      largest_label_index, largest_feature_index, max_feature_count);
...
libsvm_file::parse_contents(size_t& largest_label_index,
  ...
  while (std::getline(_stream, line, '\n')) {
    ...
    labels_and_features_row row =
        parse_row(line, rows_largest_index, max_feature_count);
...
libsvm_file::labels_and_features_row
libsvm_file::parse_row(const std::string& line, size_t& largest_feature_index,
  ...
  while (std::getline(row_stream, element, ' ')) {
    if (label_section) {
      ...
      std::smatch match;
      if (std::regex_search(element, match, feature_matcher) &&
          match.size() == 3U) {
