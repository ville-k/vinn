...
public:
  ...
  load_labels_and_features(vi::la::context& context,
                           size_t max_feature_count = 0U);

private:
  typedef std::pair<size_t, double> sparse_entry;
  typedef std::pair<std::vector<double>, std::vector<sparse_entry>>
      labels_and_features_row;
  typedef std::vector<labels_and_features_row> labels_and_features_vector;

  void fill_from_sparse_data(labels_and_features_vector sparse_data,
  ...
  labels_and_features_row parse_row(const std::string& line,
                                    size_t& largest_feature_index,
