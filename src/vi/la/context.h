...
class context {
public:
  virtual void multiply(matrix& product, const matrix& operand_1,
                        const matrix& operand_2) = 0;
  virtual void multiply(matrix& product, const matrix& operand_1,
                        const double operand_2) = 0;
  virtual void multiply_elementwise(matrix& product, const matrix& operand_1,
                                    const matrix& operand_2) = 0;

  virtual void add(matrix& sum, const matrix& operand_1,
                   const double operand_2) = 0;
  virtual void add(matrix& sum, const matrix& operand_1,
                   const matrix& operand_2) = 0;
  virtual void subtract(matrix& difference, const matrix& operand_1,
                        const matrix& operand_2) = 0;
  ...
  virtual void hyperbolic_tangent_gradient(matrix& gradient,
                                           const matrix& operand) = 0;

  virtual void softmax(matrix& operand) = 0;

  virtual void merge(matrix& merged, const matrix& operand_1,
                     const matrix& operand_2) = 0;
  ...
  virtual void sub_matrix(matrix& target, const matrix& source,
                          size_t start_row, size_t end_row, size_t start_column,
                          size_t end_column) = 0;

  virtual void convolve_2d(matrix& result, const matrix& mask,
                           const matrix& original, size_t channels) = 0;

  virtual std::shared_ptr<vi::la::matrix_implementation>
  implement_matrix(size_t rows, size_t columns,
