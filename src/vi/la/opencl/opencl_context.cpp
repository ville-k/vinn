...
void opencl_context::load_kernels() {
  ...
  _matrix_elementwise_multiply =
      new cl::Kernel(program, "matrix_elementwise_multiply");
  ...
  _hyperbolic_tangent_gradient =
      new cl::Kernel(program, "matrix_hyperbolic_tangent_gradient");
  _matrix_softmax_exp_kernel = new cl::Kernel(program, "matrix_softmax_exp");
  _matrix_softmax_normalize_kernel =
      new cl::Kernel(program, "matrix_softmax_normalize");
...
opencl_context::implement_matrix(size_t rows, size_t columns,
...
void opencl_context::multiply(matrix& product, const matrix& operand_1,
                              const matrix& operand_2) {
  opencl::matrix* product_impl = (opencl::matrix*)(product.implementation());
  opencl::matrix* operand_1_impl =
      (opencl::matrix*)(operand_1.implementation());
  opencl::matrix* operand_2_impl =
      (opencl::matrix*)(operand_2.implementation());
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_multiply_kernel, offset, size,
                                       workgroup_size);
...
void opencl_context::multiply(matrix& product, const matrix& operand_1,
  ...
  opencl::matrix* product_impl =
      dynamic_cast<opencl::matrix*>(product.implementation());
  opencl::matrix* operand_1_impl =
      dynamic_cast<opencl::matrix*>(operand_1.implementation());
...
void opencl_context::multiply_elementwise(matrix& product,
                                          const matrix& operand_1,
                                          const matrix& operand_2) {
  opencl::matrix* product_impl =
      dynamic_cast<opencl::matrix*>(product.implementation());
  opencl::matrix* operand_1_impl =
      dynamic_cast<opencl::matrix*>(operand_1.implementation());
  opencl::matrix* operand_2_impl =
      dynamic_cast<opencl::matrix*>(operand_2.implementation());
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_elementwise_multiply, offset,
                                       size, workgroup_size);
  _command_queue->finish();
}

void opencl_context::add(matrix& sum, const matrix& operand_1,
                         const double operand_2) {
  opencl::matrix* sum_impl =
      dynamic_cast<opencl::matrix*>(sum.implementation());
  opencl::matrix* operand_1_impl =
      dynamic_cast<opencl::matrix*>(operand_1.implementation());
  ...
  _command_queue->enqueueNDRangeKernel(*_scalar_add, offset, size,
                                       workgroup_size);
  _command_queue->finish();
}
...
void opencl_context::add(matrix& sum, const matrix& operand_1,
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_add, offset, size,
                                       workgroup_size);
  _command_queue->finish();
}
...
void opencl_context::subtract(matrix& difference, const matrix& operand_1,
                              const matrix& operand_2) {
  opencl::matrix* difference_impl =
      dynamic_cast<opencl::matrix*>(difference.implementation());
  opencl::matrix* operand_1_impl =
      dynamic_cast<opencl::matrix*>(operand_1.implementation());
  opencl::matrix* operand_2_impl =
      dynamic_cast<opencl::matrix*>(operand_2.implementation());
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_subtract, offset, size,
                                       workgroup_size);
  _command_queue->finish();
}
...
void opencl_context::sigmoid_gradient(matrix& gradient, const matrix& operand) {
  opencl::matrix* gradient_impl =
      dynamic_cast<opencl::matrix*>(gradient.implementation());
  opencl::matrix* operand_impl =
      dynamic_cast<opencl::matrix*>(operand.implementation());
...
void opencl_context::hyperbolic_tangent_gradient(matrix& gradient,
  ...
  cl::NDRange offset(0U, 0U);
  cl::NDRange size(operand.row_count(), operand.column_count());
  _command_queue->enqueueNDRangeKernel(*_hyperbolic_tangent_gradient, offset,
                                       size);
  _command_queue->finish();
}
...
void opencl_context::softmax(matrix& operand) {
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_softmax_exp_kernel, offset,
                                       exp_size);
  ...
  _command_queue->enqueueNDRangeKernel(*_matrix_softmax_normalize_kernel,
                                       offset, normalize_size);

  _command_queue->finish();
}

void opencl_context::merge(matrix& merged, const matrix& operand_1,
                           const matrix& operand_2) {
  opencl::matrix* merged_impl =
      dynamic_cast<opencl::matrix*>(merged.implementation());
  opencl::matrix* operand_1_impl =
      dynamic_cast<opencl::matrix*>(operand_1.implementation());
  opencl::matrix* operand_2_impl =
      dynamic_cast<opencl::matrix*>(operand_2.implementation());
...
void opencl_context::transpose(matrix& transposed, const matrix& original) {
  opencl::matrix* transposed_impl =
      dynamic_cast<opencl::matrix*>(transposed.implementation());
  opencl::matrix* original_impl =
      dynamic_cast<opencl::matrix*>(original.implementation());
...
matrix opencl_context::sum_rows(const matrix& original) {
  ...
  opencl::matrix* sum_impl =
      dynamic_cast<opencl::matrix*>(sums.implementation());
  opencl::matrix* original_imp =
      dynamic_cast<opencl::matrix*>(original.implementation());
...
void opencl_context::log(matrix& result, const matrix& original) {
  opencl::matrix* result_impl =
      dynamic_cast<opencl::matrix*>(result.implementation());
  opencl::matrix* original_imp =
      dynamic_cast<opencl::matrix*>(original.implementation());
...
void opencl_context::sub_matrix(matrix& target, const matrix& original,
                                size_t start_row, size_t end_row,
                                size_t start_column, size_t end_column) {
  opencl::matrix* target_impl =
      dynamic_cast<opencl::matrix*>(target.implementation());
  opencl::matrix* original_imp =
      dynamic_cast<opencl::matrix*>(original.implementation());
  ...
  cl_int error = _command_queue->enqueueCopyBufferRect(
      *original_imp->get(), *target_impl->get(), source_origin,
      destination_origin, region, original.column_count() * sizeof(double), 0U,
      target.column_count() * sizeof(double), 0U);
  assert(error == CL_SUCCESS);
  _command_queue->finish();
}

void opencl_context::convolve_2d(matrix& result, const matrix& mask,
                                 const matrix& original, size_t channels) {
  opencl::matrix* result_impl =
      dynamic_cast<opencl::matrix*>(result.implementation());
  opencl::matrix* mask_impl =
      dynamic_cast<opencl::matrix*>(mask.implementation());
  opencl::matrix* original_impl =
      dynamic_cast<opencl::matrix*>(original.implementation());
  ...
  const size_t INPUT_TILE_WIDTH =
      OUTPUT_TILE_WIDTH + 2 * MASK_RADIUS * channels;
  ...
  _convolve_2d->setArg(
      8U, cl::Local(INPUT_TILE_HEIGHT * INPUT_TILE_WIDTH * sizeof(cl_double)));
  ...
  const size_t horizontal_groups =
      ceil(((double)data_width) / OUTPUT_TILE_WIDTH);
  const size_t vertical_groups =
      ceil(((double)data_height) / OUTPUT_TILE_HEIGHT);

  cl::NDRange offset(0, 0);
  cl::NDRange items(vertical_groups * INPUT_TILE_HEIGHT,
                    horizontal_groups * INPUT_TILE_WIDTH);
  cl::NDRange items_per_group(INPUT_TILE_HEIGHT, INPUT_TILE_WIDTH);

  _command_queue->enqueueNDRangeKernel(*_convolve_2d, offset, items,
                                       items_per_group);
  _command_queue->finish();
}
