...
std::ostream& operator<<(std::ostream& stream, const cl::Device& device) {
  ...
  stream << "  - max work item dimensions: "
         << device.getInfo<CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS>() << std::endl;
  stream << "  - max clock freq: "
         << device.getInfo<CL_DEVICE_MAX_CLOCK_FREQUENCY>() << std::endl;
  stream << "  - max alloc size: "
         << device.getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>() / (1024 * 1024)
         << " MB" << std::endl;
  stream << "  - global mem size: "
         << device.getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() / (1024 * 1024) << " MB"
         << std::endl;

  stream << "  - local mem size: " << device.getInfo<CL_DEVICE_LOCAL_MEM_SIZE>()
         << " B" << std::endl;
  stream << "  - constant mem size: "
         << device.getInfo<CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE>() << " B"
         << std::endl;

  stream << "OpenCL: version: " << device.getInfo<CL_DEVICE_OPENCL_C_VERSION>()
         << std::endl;
